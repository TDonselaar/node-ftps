...
FTP.prototype.initialize = function (options) {
  ...
  if (opts.username && opts.requiresPassword === true && !opts.password)
    throw new Error("You need to set a password.");
  if (opts.protocol && typeof opts.protocol !== "string")
    throw new Error("Protocol needs to be of type string.");
  if (opts.requireSSHKey === true && !opts.sshKeyPath)
    throw new Error("You need to set a ssh key path.");
...
FTP.prototype.prepareLFTPOptions = function () {
  ...
  if (
    (this.options.protocol.toLowerCase() === "sftp" ||
      this.options.protocol.toLowerCase() === "fish") &&
    this.options.autoConfirm
  ) {
    opts.push(
      "set " + this.options.protocol.toLowerCase() + ":auto-confirm yes"
    );
  }
  // Only support SFTP for openSSH key authentication
  if (
    this.options.protocol.toLowerCase() === "sftp" &&
    this.options.requireSSHKey
  ) {
    opts.push(
      'set sftp:connect-program "ssh -a -x -i ' + this.options.sshKeyPath + '"'
    );
  }
  ...
  opts.push(
    "set net:reconnect-interval-multiplier " +
...
FTP.prototype.exec = function (cmds, callback) {
  ...
  var lftp = spawn(this.options.binaryPath + "lftp", ["-c", cmd], spawnoptions);
  var data = "";
  var error = "";
...
FTP.prototype.execAsStream = function (cmds) {
  ...
  return dcp.spawn(this.options.binaryPath + "lftp", ["-c", cmd], spawnoptions);
};
...
FTP.prototype.mv = function (from, to) {
  ...
  return this.raw(
    "mv " + this._escapeshell(from) + " " + this._escapeshell(to)
  );
};
  ...
  return this.raw(
    "rm " +
      Array.prototype.slice
        .call(arguments)
        .map(this._escapeshell.bind(this))
        .join(" ")
  );
};
...
FTP.prototype.mirror = function (opts) {
  ...
  if (opts.upload === true) {
    raw +=
      " " +
      this._escapeshell(opts.localDir) +
      " " +
      this._escapeshell(opts.remoteDir);
  } else {
    raw +=
      " " +
      this._escapeshell(opts.remoteDir) +
